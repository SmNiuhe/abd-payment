...
import static com.fangxie.payment.enums.WxpayOrderStatusEnum.PRECREATE_FAILED;
...
public class WxPayController {
    ...
    public ResultBody tradePrecreate(@RequestBody WxPayUnifiedOrderDTO wxPayUnifiedOrderDTO) {

        try {
            return wxPayService.createOrder(wxPayUnifiedOrderDTO);
        } catch (WxPayException e) {
            return ResultBody.body(PRECREATE_FAILED, e.getReturnMsg());
        }
    }

    @GetMapping("/trade/pre_create")
    public ResultBody tradePrecreateDefault() {

        try {
            return wxPayService.createOrder(null);
        } catch (WxPayException e) {
            return ResultBody.body(PRECREATE_FAILED, e.getReturnMsg());
        }
    }
    ...
    public ResultBody queryOrder(@RequestBody WxPayQueryOrderDTO wxPayQueryOrderDTO)
            throws WxPayException {

        return wxPayService.queryOrder(wxPayQueryOrderDTO);
    }
    ...
    public ResultBody refund(@RequestBody WxPayRefundDTO wxPayRefundDTO) throws WxPayException {


        WxPayConfig config = wxService.getConfig();
        System.out.println(config.getKeyPath());
     ...
     * @return
     */
    @PostMapping("/trade/refund-query")
    public ResultBody refundQuery(@RequestBody WxPayRefundQueryDTO wxPayRefundQueryDTO) {

        return wxPayService.refundQuery(wxPayRefundQueryDTO);
    }
    ...
    @PostMapping("/createScanPayQrcodeMode1/byte")
    public byte[] createScanPayQrcodeMode1_byte(String productId, File logoFile, Integer sideLength) {
        return this.wxService.createScanPayQrcodeMode1(productId, logoFile, sideLength);
    }
    ...
    @PostMapping("/createScanPayQrcodeMode1/string")
    public String CreateScanPayQrcodeMode1_string(String productId) {
        return this.wxService.createScanPayQrcodeMode1(productId);
    }
    ...
    public byte[] testCreateScanPayQrcodeMode2(String codeUrl, MultipartFile logoFile, Integer sideLength) {

        File file = null;
        try {
            file = new File(fileProperties.getTemp(), logoFile.getOriginalFilename());
            logoFile.transferTo(file);
        } catch (IOException e) {
            e.printStackTrace();
        }

        return this.wxService.createScanPayQrcodeMode2(codeUrl, file, sideLength);
    }
